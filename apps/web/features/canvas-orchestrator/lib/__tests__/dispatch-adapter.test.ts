...
describe("runtime version pre-flight (quick-create gate)", () => {
  ...
  describe("checkAgentDispatchable", () => {
    it("passes a `dev` cli_version runtime (matches server source-build gate)", () => {
  ...
  describe("resolveDispatchableAgentForExecutor", () => {
    it("prefers a qualified same-executor agent over an unqualified one", () => {
      const agents = [
        // First match by hint and its runtime is a source build → qualified.
        agent({ id: "a1", name: "Claude A", model: "claude-opus-4", runtime_id: "r-dev" }),
        // Second same-executor match is also qualified, but lower preference.
      ...
      expect(res.ok && res.agent.id).toBe("a1");
    });

    it("blocks (ok:false) with current/min when no agent qualifies", () => {
      const agents = [
        agent({ id: "a1", name: "Claude", model: "claude-opus-4", runtime_id: "r-old" }),
      ];
      const res = resolveDispatchableAgentForExecutor(
        "claude",
        agents,
      ...
      if (!res.ok) {
        ...
        expect(res.runtimeId).toBe("r-old");
        expect(res.current).toBe("0.1.0");
  ...
  describe("preflightLineDispatch", () => {
    it("returns no blocks when the only online runtimes are source-build `dev` runtimes", () => {
      const agents = [
    ...
    it("gates dispatch: a blocked pre-flight never reaches quickCreateIssue", async () => {
      ...
      // when there are no blocks. With a too-old release runtime, the line must be
      // refused before any issue/task is created.
      const agents = [
        agent({ id: "a1", name: "Claude", model: "claude-opus-4", runtime_id: "r-old" }),
      ];
      const runtimes = rtMap(runtime("r-old", "online", "0.1.0"));
