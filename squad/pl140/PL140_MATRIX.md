...
| X47 | ✅ 端到端补齐 | `line_observe.py::emit_observability` → `memory/.evidence/cycles.jsonl`;`run_watchdog.sh --emit-observability` | 实盘 `cycles.jsonl`(184 行,08:47 更新);wiring 见 `run_watchdog.sh:60` | PL-126「未接 cron」判定已过时——周期账本现已逐轮落盘。判定纠偏为 PASS |
...
| X63 | ✅ 端到端补齐 | `line_partial.py heartbeat-check` CLI + `systemd/line-watchdog-heartbeat.{service,timer}` | `::X63HeartbeatAlert`(3 例)+ CLI 真跑 stale→exit 2 / fresh→exit 0(见 README) | 进程外 watcher:看门狗自身死掉它仍跑,停摆→落 fallback 告警 + exit 2。systemd 装载是运维一步(`systemd/install.sh`) |
