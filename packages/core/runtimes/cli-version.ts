...
// A clean release is the only shape we rank numerically against the minimum.
// Anything else non-empty is a dev / HEAD / source build. This mirrors the
// server gate: production daemons built from source may report bare "dev".
const CLEAN_RELEASE_RE = /^v?\d+\.\d+\.\d+$/;
 ...
 * `"missing"` for empty input (fail closed — same policy as the server) and
 * `"too_old"` for a clean release below the threshold. Non-release strings
 * (`dev`, git-describe output, commit hashes, dirty builds) are source/HEAD
 * builds and pass the gate.
 */
export function checkQuickCreateCliVersion(detected: string | undefined | null): CliVersionCheck {
  ...
  if (!CLEAN_RELEASE_RE.test(current)) {
