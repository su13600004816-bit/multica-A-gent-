...
func (h *Handler) GetSquadCanvasLayout(w http.ResponseWriter, r *http.Request) {
	squadUUID, ok := parseUUIDOrBadRequest(w, chi.URLParam(r, "id"), "squad id")
	if !ok {
		return
	}
	var layout []byte
	err := h.DB.QueryRow(r.Context(),
		`SELECT layout FROM squad_canvas_layout WHERE squad_id = $1`, squadUUID).Scan(&layout)
...
func (h *Handler) SetSquadCanvasLayout(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.DB.Exec(r.Context(),
		...
		squadUUID, []byte(body.Layout)); err != nil {
