...
# 信封协议(新):专属审计员以 `[MSG] AUDIT_PASS / AUDIT_FAIL` 信封头发结论(见各 C0x-R 审计 skill)。
# 这是与旧 `VERDICT: PASS` 等价的审计结论声明,门禁必须互认,否则审计已 PASS 也签不掉(协议代差死循环根因)。
# 只认结论位(行首,前可有反引号);线主脑 `[MSG] CLOSE_PASS` 非审计结论,不匹配 -> 不破坏异模型交叉审计独立性。
_ENVELOPE_AUDIT_RE = re.compile(r'(?:^|\n)\s*`?\s*\[MSG\]\s*AUDIT_(PASS|FAIL)\b', re.IGNORECASE)

def envelope_audit_verdict(text):
    """返回信封协议审计结论里最后一个 AUDIT_(PASS|FAIL),无则 None。仅供 audit_pass 条使用。"""
    v = None
    for m in _ENVELOPE_AUDIT_RE.finditer(text or ""):
        v = m.group(1).upper()
    return v
...
def evaluate(issue, comments, runs=None):
    ...
    # 2. audit_pass:最后一个出现的审计结论(排除门禁回写),且不是 FAIL。
    #    认两种等价结论格式:旧 `VERDICT: PASS`(独占一行)与 新信封 `[MSG] AUDIT_PASS`。
    audit_last = None
    for c in comments:
        t = ctext(c)
        if is_real_gate_comment(t):
            continue  # 门禁的 verdict 单独算
        v = standalone_verdict(t) or envelope_audit_verdict(t)
        if v:
            audit_last = v
    ap = audit_last == "PASS"
    reasons["audit_pass"] = (ap, "专属审计最新结论 PASS(VERDICT/AUDIT_PASS)" if ap else "专属审计未 PASS(最新=%s)" % (audit_last or "无"))
