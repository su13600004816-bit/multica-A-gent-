...
_AUDIT_SRC_RE = re.compile(r"(专属审计|审计\s*codex|\baudit\b|审计结论|VERDICT.*审计|\[MSG\]\s*AUDIT_(?:PASS|FAIL))", re.IGNORECASE)
...
# C0x 编制审计员身份(新生产线):岗位 R = 专属审计员,名形如 "C01-R 审计员"…"C06-R 审计员"。
# 按身份采信(不靠文本),与旧 "审计Codex-T01" 同级,堵协议代差又不放开自审口子。
_AUDIT_AGENT_RE = re.compile(r"\bC\d{2}-R\b", re.IGNORECASE)
...
def classify_source(text, agent=None):
    ...
    if a in TRUSTED_GATE_AGENTS:
        return "qwen_gate"
    if a in TRUSTED_AUDIT_AGENTS or _AUDIT_AGENT_RE.search(a):
        return "codex_audit"
    if is_self_report(t):
        return "self_report"
...
# ─────────────────────────────────────────────────────────────────────────────
# B-PL142:结构化 run_role / evidence_type —— 按"线小队标准流程"的岗位把一次 run
# 归类成 coordination/development/audit/root_cause/gate/acceptance/crisis,供看门狗
# 据此判定能否当门禁/收口证据、能否覆盖旧 PASS,**不再靠评论文本猜**(返工要求②)。
#
# 设计:身份(岗位名/触发类型)优先,文本只在身份完全缺失时作最弱兜底。这样
#   - 线主脑协调 run / 纯派发 / 个人 mention / no_action -> coordination,绝不当门禁证据;
#   - 写 Claude run -> development(dev evidence),不可覆盖旧 PASS;
#   - Codex 审 -> audit;DeepSeek 深挖 -> root_cause;千问门禁 -> gate;
#   - cx Z Fold7 / 真机截图验收 -> acceptance;危机处理 -> crisis。
# ─────────────────────────────────────────────────────────────────────────────
RUN_ROLE_COORDINATION = "coordination"
RUN_ROLE_DEVELOPMENT  = "development"
RUN_ROLE_AUDIT        = "audit"
RUN_ROLE_ROOT_CAUSE   = "root_cause"
RUN_ROLE_GATE         = "gate"
RUN_ROLE_ACCEPTANCE   = "acceptance"
RUN_ROLE_CRISIS       = "crisis"
RUN_ROLE_UNKNOWN      = "unknown"

# run_role -> evidence_type(看门狗据此判定证据效力)
RUN_ROLE_EVIDENCE = {
    RUN_ROLE_COORDINATION: "coordination",   # 仅协调,绝不作门禁/收口证据
    RUN_ROLE_DEVELOPMENT:  "development",     # 仅开发自证(dev evidence),非门禁
    RUN_ROLE_AUDIT:        "audit",           # 审计证据
    RUN_ROLE_ROOT_CAUSE:   "root_cause",      # 深挖根因证据
    RUN_ROLE_GATE:         "gate",            # 门禁证据(可信)
    RUN_ROLE_ACCEPTANCE:   "acceptance",      # 真机/截图验收证据(可信)
    RUN_ROLE_CRISIS:       "crisis",          # 危机处理动作,非门禁证据
    RUN_ROLE_UNKNOWN:      "self_report",     # 身份不明,按自报兜底(非门禁)
}

# 可作为"可信门禁/收口"证据、且能覆盖旧 PASS 的 evidence_type。
GATE_EVIDENCE_TYPES = {"gate", "audit", "acceptance"}
# 绝不可当门禁证据、绝不可覆盖旧 PASS 的 run_role(协调/纯派发/危机动作)。
NON_GATE_RUN_ROLES = {RUN_ROLE_COORDINATION, RUN_ROLE_CRISIS}

# 身份(岗位名)-> run_role。顺序敏感:越具体的岗位越靠前匹配,避免
# "审计Codex" 命中 development、"线主脑"(codex)命中 audit 之类误判。
_ROLE_BY_AGENT = [
    (re.compile(r"(危机|crisis)", re.IGNORECASE),                              RUN_ROLE_CRISIS),
    (re.compile(r"(深挖|根因|deepseek|deep[\s_-]?dig|root[\s_-]?cause)", re.IGNORECASE), RUN_ROLE_ROOT_CAUSE),
    (re.compile(r"(验收|真机|z\s*fold|fold\s*7|拟人|playwright|截图|\bcx\b)", re.IGNORECASE), RUN_ROLE_ACCEPTANCE),
    (re.compile(r"(门禁|千问|\bqwen\b|\bgate\b)", re.IGNORECASE),               RUN_ROLE_GATE),
    (re.compile(r"(审计|审\s*codex|codex.*审|\baudit\b)", re.IGNORECASE),       RUN_ROLE_AUDIT),
    (re.compile(r"(主脑|线主脑|line[\s_-]?brain|协调|coordination|调度|派发|dispatch)", re.IGNORECASE), RUN_ROLE_COORDINATION),
    (re.compile(r"(写\s*claude|开发|claude.*开发|开发.*claude|development|\bdev(eloper)?\b)", re.IGNORECASE), RUN_ROLE_DEVELOPMENT),
]

# 触发类型关键词 -> 一律 coordination(纯派发/个人 mention/no_action 不当门禁证据)。
_COORD_TRIGGER_RE = re.compile(
    r"(dispatch|派发|mention|no[\s_-]?action|coordination|协调|assign|reassign|续派)",
    re.IGNORECASE)

# 文本兜底来源 -> run_role(仅身份完全缺失时才用)。
_SOURCE_TO_ROLE = {
    "qwen_gate": RUN_ROLE_GATE,
    "codex_audit": RUN_ROLE_AUDIT,
    "playwright_artifact": RUN_ROLE_ACCEPTANCE,
    "self_report": RUN_ROLE_DEVELOPMENT,
    "comment_claim": RUN_ROLE_DEVELOPMENT,
}


def _run_agent_name(run, agent):
    """从 run 记录里取出执行方身份名(尽量不依赖单一字段名)。"""
    if agent:
        return str(agent).strip()
    run = run or {}
    for k in ("agent_name", "agent", "assignee_name", "actor", "triggered_by",
              "role", "member_name"):
        v = run.get(k)
        if v:
            return str(v).strip()
...
def classify_run_role(run=None, agent=None, text=""):
    """B-PL142:把一次 run 归类成结构化 run_role(状态机岗位)。

    判定优先级(身份优先、文本最弱):
      1. 执行方岗位名(agent/run 身份)命中 _ROLE_BY_AGENT -> 直接定岗。
      2. 触发类型为纯派发/个人 mention/no_action/协调 -> coordination。
      3. 身份完全缺失时,才弱看文本来源(真门禁/审计/真机回写)兜底。
    返回 RUN_ROLE_* 之一。
    """
    name = _run_agent_name(run, agent)
    for rx, role in _ROLE_BY_AGENT:
        if name and rx.search(name):
            return role
    run = run or {}
    trig = " ".join(str(run.get(k, "")) for k in
                    ("trigger", "trigger_type", "reason", "kind", "source"))
    if trig.strip() and _COORD_TRIGGER_RE.search(trig):
        return RUN_ROLE_COORDINATION
    if text:
        return _SOURCE_TO_ROLE.get(classify_source(text, agent=name), RUN_ROLE_UNKNOWN)
    return RUN_ROLE_UNKNOWN


def run_evidence_type(role):
    """run_role -> evidence_type。"""
    return RUN_ROLE_EVIDENCE.get(role, "self_report")


def run_role_is_gate_evidence(role):
    """该 run_role 是否构成可信门禁/收口证据(能覆盖旧 PASS)。"""
    if role in NON_GATE_RUN_ROLES:
        return False
    return run_evidence_type(role) in GATE_EVIDENCE_TYPES


def classify_run_evidence(run=None, agent=None, text=""):
    """一次性给出结构化分类,供看门狗/门禁直接消费。

    {run_role, evidence_type, is_gate_evidence, can_supersede_pass}
    - is_gate_evidence/can_supersede_pass:仅 gate/audit/acceptance 为真;
      coordination/development/root_cause/crisis/unknown 一律为假
      —— 杜绝"线主脑协调/纯派发/个人 mention/危机动作"覆盖旧 PASS。
    """
    role = classify_run_role(run, agent, text)
    etype = run_evidence_type(role)
    gate = run_role_is_gate_evidence(role)
    return {
        "run_role": role,
        "evidence_type": etype,
        "is_gate_evidence": gate,
        "can_supersede_pass": gate,
    }
...
def run_evidence_role(run):
    ...
    ev = classify_run_evidence(run=run, text=blob)
    name = _run_agent_name(run, None)
    trig = " ".join(str(run.get(k, "")) for k in
                    ("trigger", "trigger_type", "reason", "kind", "source"))
    structured_non_gate = bool(name or _COORD_TRIGGER_RE.search(trig or ""))
    if structured_non_gate and not ev["can_supersede_pass"]:
        role = ev["run_role"]
        etype = ev["evidence_type"]
        return _r(etype, "%s run 只作为 %s evidence,不得作为门禁证据覆盖旧 PASS" % (
            role, etype), gate_relevant=False, verdict=None,
