...
LINE_SQUAD_RE = re.compile(r"^线小队-[TC]\d+$")  # 兼容 T 线(旧)与 C0x 线(章程编制)
...
_COMM_LIST_FX = None
_PASS_RE = re.compile(r"VERDICT[:：\s]*PASS|门禁\s*PASS|AUDIT_PASS|\bPASS\b", re.IGNORECASE)
_DONEREAL_RE = re.compile(r"DONE_REAL|VERDICT[:：\s]*PASS|门禁\s*PASS|AUDIT_PASS", re.IGNORECASE)
_VERDICT_FAIL_RE = re.compile(r"VERDICT[:：\s]*FAIL|门禁[:：\s]*FAIL|AUDIT_FAIL", re.IGNORECASE)
...
KIND_META = {
    ...
    "done_no_gate":     ("brain",  "done 但无真门禁裁决(疑似绕门禁收口)"),          # B-done源头
...
# 易变片段:经过时长("已 40 分钟"/"5m"/"2.0小时")与 ISO 时间戳 —— 每轮都涨,
# 不是有意义的状态变化。必须从去重签名里剔除,否则同一持续告警 sig 每 2 分钟都变,
# 去重永远命中不了 -> 每周期重贴 = 刷屏(看门狗被停用的真凶)。
_SIG_VOLATILE_RE = re.compile(
    r"\d+(?:\.\d+)?\s*(?:分钟|分|秒钟|秒|小时|天)"               # 中文时长(后接中文,不能要 \b)
    r"|\d+(?:\.\d+)?\s*(?:minutes?|mins?|hours?|hrs?|[mhs])\b"    # 英文时长(要词边界,避免误吞)
    r"|\d{4}-\d{2}-\d{2}T[\d:.+\-Z]+",                           # ISO 时间戳
    re.IGNORECASE)

def alert_sig(al):
    """单条告警的去重签名:why 含状态(连续取消次数等),状态变化->sig 变->重新提醒。
...
def dedup_changed(alerts, prev_sigs, route_retry, now):
    ...
    去重原则:
      - sig 变化 -> changed(状态变了,重新提醒/重派)。
      - sig 不变但该 key 处于 ROUTE_UNCONFIRMED 退避、且 next_retry_ts 已到、退避未耗尽
        -> changed(到点补派一次,随后退避指数加长)。
      - sig 不变且仍在退避冷却窗内(或退避已耗尽放弃自动补派)-> **不** changed。

    关键修复(返工要求①):续派未确认时不再无条件清掉去重签名。旧实现一旦
    ROUTE_UNCONFIRMED 就 pop(alert_sigs[key]),导致下一轮 sig!=旧 -> changed ->
    每 2 分钟对同一 gate_fail_rework/evidence_missing 重复 POST/route 刷屏死循环。
    现在保留签名、靠 route_retry 的 next_retry_ts 退避补派,彻底消除刷屏。
    """
    route_retry = route_retry or {}
    changed, new_sigs = [], {}
    for al in alerts:
        key = alert_state_key(al)
        sig = alert_sig(al)
        new_sigs[key] = sig
        if prev_sigs.get(key) != sig:
            changed.append(al)
            continue
        rr = route_retry.get(key)
        if (rr and rr.get("reason") == "unconfirmed"
                and not O.retry_exhausted(rr) and O.retry_due(rr, now)):
            changed.append(al)
    return changed, new_sigs
...
def alert_hint(al):
    ...
    return {
        ...
        "done_no_gate": "issue 已 done 但全程没有任何带 VERDICT 的真门禁/审计裁决(PASS 或 FAIL 都没有),评论里也无可信门禁/审计 PASS 背书,疑似绕过门禁直接收口。应补走真门禁(line_bridge.py gate)或专属审计复核留下可验证证据;未过真门禁不得 done。",
...
def infer_squad_from_issue(issue):
    ...
    m = re.search(r"(?:^|[^A-Za-z0-9])([TC])0?(\d{1,2})(?:[^0-9]|$)", text)
    if not m:
        return None
    wanted = "线小队-%s%02d" % (m.group(1).upper(), int(m.group(2)))
...
def detect(items, zombie_min, stale_min, recent_min=180.0, active_subset_ids=None,
    ...
    for i in owned_issues(items, DONE_ST):
        ...
        if i.get("status") == "done":
            ...
            # B(done 源头可视化):done 但无任何带 VERDICT 的真门禁裁决(no_evidence/evidence_missing)
            # = 最赤裸的绕门禁收口。audit_issue_evidence 只判 PASS/FAIL 锚点,这两种结论交给 BOM-2,
            # 而 BOM-2 只扫 ACTIVE_ST、done 不在内 -> 这类裸 done 会完全静默。这里补专项告警:
            # 限近期 done(上面 TERMINAL_SCAN_MIN 已截断)、有 run 却无门禁结论(零 run 的协调类不报)、
            # 且评论也无可信门禁/审计 PASS 背书。why 用稳定措辞 -> alert_sig 归一化后跨周期稳定,不刷屏。
            if runs and E.evidence_gate_decision(runs)["status"] in ("no_evidence", "evidence_missing") \
                    and not comment_has_trusted_pass(i.get("id")):
                alerts.append(_mk("done_no_gate", i, None,
                    "issue 已 done 但无任何带 VERDICT 的真门禁/审计裁决,且最近评论无可信门禁/审计 PASS 背书,疑似绕门禁直接收口"))
    return alerts
...
def main():
    ...
    # ===== 去重(BOM-7 + B-PL142):按 (ident|kind) 分别签名 =====
    # B-PL142:dedup_changed 现在还会读 route_retry —— ROUTE_UNCONFIRMED 退避到点才
    # 把同一告警重新放进 changed 补派一次,杜绝每轮重复 POST/route 刷屏(返工要求①)。
    sigs = st["alert_sigs"]
    changed, new_sigs = dedup_changed(alerts, sigs, st.get("route_retry", {}), now_utc())
    ...
    if a.route and not scan_blocked and route_coord_fd is not None:
        ...
        for res in results:
            ...
            if res["ok"]:
                ...
                if res.get("confirmed") is True:
                    ...
                    # 确认有新 run 启动 -> 清掉该 key 的未确认退避(闭环成功)
                    route_retry = st.get("route_retry", {})
                    for al in job["alerts"]:
                        route_retry.pop(alert_state_key(al), None)
                elif res.get("confirmed") is False:
                    print("ROUTE_UNCONFIRMED %s %s" % (job["ident"], res.get("confirm_info", "")))
                    # B-PL142 修复(返工要求①):未确认时**不再** pop alert_sigs ——
                    # 保留去重签名,改为给该 key 装/续指数退避(首轮 2 分钟、封顶 30 分钟)。
                    # dedup_changed 只在 next_retry_ts 到点且退避未耗尽时才补派一次,
                    # 杜绝同一 gate_fail_rework/evidence_missing 每 2 分钟重复 POST/route。
                    route_retry = st.setdefault("route_retry", {})
                    for al in job["alerts"]:
                        key = alert_state_key(al)
                        meta = O.route_retry_meta(
                            route_retry.get(key), now_utc(),
                            base_backoff=O.UNCONFIRMED_BACKOFF_BASE,
                            cap=O.UNCONFIRMED_BACKOFF_CAP, reason="unconfirmed")
                        route_retry[key] = meta
                        if O.retry_exhausted(meta):
                            print("ROUTE_UNCONFIRMED_GIVEUP %s failcount=%d -> 停止自动补派(保留贴台,转人工/危机)" % (
                                job["ident"], meta["failcount"]))
                        else:
                            print("ROUTE_UNCONFIRMED_BACKOFF %s failcount=%d next=%s" % (
                                job["ident"], meta["failcount"], meta["next_retry_ts"]))
                for al in job["alerts"]:
        ...
        if retry_keys:
            ...
            for key in retry_keys:
                ...
                # B-PL142:标记 reason=route_failed,与 unconfirmed 退避区分,
                # 避免 dedup_changed 误把 route 执行失败的退避当未确认补派。
                meta = O.route_retry_meta(route_retry.get(key), now_utc(), reason="route_failed")
                route_retry[key] = meta
